...
public class PhotoUtil {
    ...
    * */
    ...
    private static int time = Constants.TIME;//s
    ...
    //倒计时
    public static void resetCode(final TextView tv) {
        handler = new Handler();
        t = new Thread() {
            @Override
            public void run() {
    ...
    public static Uri getUriForFile(Context context, File file) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            uri = FileProvider.getUriForFile(context, Constants.package_fileProvider, file);
        } else {
