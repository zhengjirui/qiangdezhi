 ...
 * Author: LGH
 * Date: 2017/12/15
 * Description: copy for SpannelTextViewNew
 */

public class SpannelTextViewGrid extends View {
    ...
    public SpannelTextViewGrid(Context context) {
        this(context, null);
    }

    public SpannelTextViewGrid(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SpannelTextViewGrid(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        //设置字体的颜色
        paint.setColor(getResources().getColor(R.color.c_494949));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mNewLine) {
            ...
            canvas.drawText(second, lp.leftMargin, 5 * scaleDenisty + (3f * scaleDenisty + 0.5f) + lp.topMargin +
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        ViewGroup.MarginLayoutParams lp = (ViewGroup.MarginLayoutParams) getLayoutParams();
        if (bitmap.getWidth() + mTextWidth + lp.leftMargin + lp.rightMargin + 25 > width) {
            //换行
            for (int x = 0, length = drawText.length(); x < length; x++) {
                ...
                if (bitmap.getWidth() + v + lp.leftMargin + lp.rightMargin >= width) {
                    ...
                    if (drawText.length() > x + 12) {
                        second = drawText.substring(x - 1, x + 12) + "...";
                    } else {
        ...
        // 此处数值,调整控件的高度.
        setMeasuredDimension(width, (int) (30 * scaleDenisty + 0.5f));
    }
